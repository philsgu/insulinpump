...
theme_neutral = {'bgcolor': '#f9f9f9', 'title_color': 'orange',
                 'content_color': 'orange', 'icon_color': 'orange', 'icon': 'fa fa-star-of-life'}
theme_good = {'bgcolor': '#EFF8F7', 'title_color': 'green',
              'content_color': 'green', 'icon_color': 'green', 'icon': 'fa fa-syringe'}
theme_ok = {'bgcolor': '#e8e6f7', 'title_color': 'purple',
            'content_color': 'purple', 'icon_color': 'purple', 'icon': 'fa fa-utensils'}
...
def dosage(tdd, rate=False, fixed=False):
    if rate:
...
insulin_pump, quick_bolus, summary = st.tabs(
    ['Calculate Inital Pump Settings', 'Quick Bolus Calculator', 'Summary Sheet'])
...
with insulin_pump:
    st.number_input(label="Prepump Total Daily Dose",
                    key='pre_pump', min_value=0, max_value=500, step=1)
    st.number_input(label="Weight (lbs)", key='weight',
    ...
    if st.session_state.pre_pump and not st.session_state.weight:
        ...
        st.caption(
            "**About Method 1:**  tDD = pre-pump TDD x 0.75; Reduce Pre-Pump by 25% or less if pre-pump TDD >70%")
    ...
    if st.session_state.weight and not st.session_state.pre_pump:
        ...
        "#### Total Basal Daily Dosage"
        st.warning(
            f"**{dosage(wt_tdd, False, False)} units/day (total daily basal insulin)**")
    ...
    if st.session_state.pre_pump and st.session_state.weight:
        ...
        # calculate average
        st.success(
            f"Average of pre-pump TDD and weight based: **{avg_tdd}** units/day")
        ...
        "#### Total Basal Daily Dosage"
        st.warning(
            f"**{dosage(avg_tdd, False, False)} units/day (total daily basal insulin)**")
...
# QUICK BOLUS
with quick_bolus:
    carbs = st.number_input("**Enter carb units (g)**",
                            min_value=0, step=1, key='carbs')
    qicr = st.number_input("**Enter carb ratio**", min_value=0, step=1,
                           value=m1_carb_ratio, key='qicr', help='Default value using 450 carb ratio rule')
    qisf = st.number_input("**Enter insulin correction factor (mg/dL/unit)**",
                           min_value=0, step=1, value=isf, key="qisf")
    target_bg = st.number_input("**Enter target Blood Gluose (mg/dL)**",
                                min_value=80, max_value=180, step=1, key='target_bg')
    ...
    active_insulin = st.selectbox(
        "**Select Active Insulin Time**", options=(3, 4, 5))
    with st.expander(label="**Suggested Active Insulin Time**"):
        ...
        with col3:
            st.write('### Pregnancy')
            st.write("3-4 hours")
    glucose_read = st.number_input(
        label="Enter current glucose reading (mg/dL)", min_value=0, max_value=800, step=1)
    ...
    iob = round(active_insulin_amount(
        basal_rate=basal_rate, active_time=active_insulin), 1)
    ...
    with col1:
        ...
        hc.info_card(title="Recommended Correction (units)", content=suggest_correction,
                     theme_override=theme_neutral, title_text_size="1rem", content_text_size="2rem")
    with col3:
        # st.metric(label="**Total Insulin (units)**", value=round((insulin_carbs+suggest_correction), 1))
        hc.info_card(title="Total Insulin Required (units)", content=total_bolus,
                     theme_override=theme_good, title_text_size="1rem", content_text_size="2rem")
    with col2:
        # st.metric(label="Food Bolus Insulin (units)", value=insulin_carbs)
        hc.info_card(title="Food Bolus Insulin (units)", content=insulin_carbs,
                     theme_override=theme_ok, title_text_size="1rem", content_text_size="2rem")
...
with summary:
    ...
    with col1:
        ...
        dob = st.date_input('DOB', min_value=datetime.date.today(
        )-relativedelta(years=100), max_value=datetime.date.today())
    with col2:
